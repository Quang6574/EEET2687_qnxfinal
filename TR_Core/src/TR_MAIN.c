...
int Server_Init()
{
	...
	if ((attach_train = name_attach(NULL, ATTACH_POINT_TR, 0)) == NULL)
	{
...
int Client_Init()
{
	 ...
	 printf("Connection established to: %s, %s and %s\n", GLOBAL_ATTACH_POINT_I1, GLOBAL_ATTACH_POINT_CTL, GLOBAL_ATTACH_POINT_I2);
	 return EXIT_SUCCESS;
}
...
int main(void) {
	...
	while(servcode && (restart_try > 0))
	{
		...
		servcode = Server_Init(attach_train);
		restart_try--;
	}
	...
	receiver_arg.attach = attach_train;
